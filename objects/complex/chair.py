...
class Chair(ObjectComplex):
    def __init__(self, position: np.ndarray, camera: Camera = None):
        ...
        back_supports = [Cylinder(backseat.position + np.array([-0.010, -0.20, 0.10]), axis, 0.20, 0.015, material=SUPPORT) for _ in range(2)]
        back_supports[1].translate(np.array([0., 0., 0.25]))

        right_support1 = BasedCylinder(backseat.position + np.array([-0.025, 0.06, -0.10]), np.array([0., 0., -1.]), 0.10, 0.01, material=SUPPORT, baseTop=False)
        right_support2 = BasedCylinder(backseat.position + np.array([-0.025, 0.06, -0.10]), np.array([-1., 0., 0.]), 0.25, 0.01, material=SUPPORT, baseTop=False)
